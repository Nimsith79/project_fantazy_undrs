...
- `LORA_REMOVE_CLOTHING_URL=<url>`: fallback URL for `qwen_image_edit_remove-clothing_v1.0.safetensors` if not provided locally.
- `LORA_BEAUTY10_URL=<url>`: fallback URL for `beuauty10.safetensors` if not provided locally.
...
2. Provide `LORA_REMOVE_CLOTHING_URL` and `LORA_BEAUTY10_URL` as build args.
