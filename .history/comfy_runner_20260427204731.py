...
class ComfyRunner:
    ...
    def _validate_prompt_nodes_exist(self, prompt: Dict[str, Any]) -> None:
        try:
            response = self.session.get(f"{self.base_url}/object_info", timeout=30)
        except requests.RequestException as exc:
            raise ComfyExecutionError(f"Failed to fetch ComfyUI node metadata: {exc}") from exc

        if not response.ok:
            raise ComfyExecutionError(
                f"ComfyUI node metadata request failed with status {response.status_code}."
            )

        object_info = response.json()
        if not isinstance(object_info, dict):
            raise ComfyExecutionError("Invalid node metadata received from ComfyUI.")
        ...
        if missing_nodes:
            ...
            raise WorkflowError(
                f"Missing ComfyUI node types: {joined}. Install required custom nodes and restart worker."
            )
    ...
    def run_workflow(self, image_path: str, workflow: Dict[str, Any], timeout_seconds: int) -> str:
        ...
        with self._run_lock:
            ...
            try:
                ...
                self._validate_prompt_nodes_exist(prompt)
